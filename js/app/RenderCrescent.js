...
export default class RenderCrescent extends RenderSimple {
  ...
  render() {
    ...
    if (this.particles && this.particles.length > 0) {
      ...
      this.drawCircle(ctx, this.particles[0], 200);
      this.drawCircle(ctx, this.particles[1], 280);
    }

    this.drawCrescentFromIntersections(
      this.particles[0].getX(), this.particles[0].getY(), 200,
      this.particles[1].getX(), this.particles[1].getY(), 280,
      true
    );

    this.drawEllipseAroundPoints(
      this.particles[0].getX(), this.particles[0].getY(),
      this.particles[1].getX(), this.particles[1].getY(),
      0.5, 30);
  }
  ...
  drawCrescentFromIntersections(x1, y1, r1, x2, y2, r2, showGuides = false) {
    ...
    ctx.fillStyle = '#13c6fd';
    ctx.shadowBlur = 15;
    ctx.shadowColor = '#136dfd';
    ctx.beginPath();
    // Inner arc from interP2 back to interP1 (counterclockwise)
    ...
    ctx.strokeStyle = '#06fe12';
    ctx.lineWidth = 15;
    ctx.shadowBlur = 15;
    ctx.shadowColor = '#33954d';
    ...
    ctx.arc(x1, y1, r1, angle1_outer, angle2_outer, true);
    ctx.stroke();
    ctx.fillStyle = '#fe06d9';
    ctx.fill()
    ctx.shadowBlur = 0;

    // ctx.beginPath();
    // ctx.arc(x1, y1, r1, Math.PI + angle2_inner, Math.PI + angle1_inner, true);
    // ctx.stroke();
    // ctx.shadowBlur = 0;
  ...
  drawCircle(ctx, p, r) {
    ctx.fillStyle = 'rgb(255,100,100,.2)';
