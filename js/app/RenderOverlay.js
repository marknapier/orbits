...
export default class RenderOverlay extends RenderSimple {
  ...
  render() {
    ...
    for (const p of this.particles) {
      this.drawCircle(ctx, p, p.radius * p.sizeMultiplier);
    }
    ...
    //       p1.getX(), p1.getY(), p1.radius*10,
    //       p2.getX(), p2.getY(), p2.radius*10,
    //       true
    //     );
    //   }
    // }
  ...
  drawCrescentFromIntersections(x1, y1, r1, x2, y2, r2, showGuides = false) {
    ...
    if (showGuides) {
      ...
      ctx.strokeStyle = 'rgb(123, 123, 123, 0.3)';
      ctx.beginPath();
  ...
  drawCircle(ctx, p, r) {
    ctx.strokeStyle = 'rgba(253, 76, 135, 0.7)'; //'rgba(255, 100, 196, 0.6)';
    ctx.lineWidth = 2;
    ctx.beginPath();
